...
public class FollowServiceImpl extends ServiceImpl<FollowMapper, Follow> implements IFollowService {
    ...
    public Result follow(Long followUserId, Boolean isFollow) {
        ...
        if (isFollow) {
            ...
            if (isFollow) stringRedisTemplate.opsForSet().add(key,followUserId.toString());

        } else {
            // 3.取关，删除 delete from tb_follow where user_id = ? and follow_user_id = ?
            boolean isSuccess = remove(new LambdaQueryWrapper<>(Follow.class).eq(Follow::getUserId, userId).eq(Follow::getFollowUserId, followUserId));
            //如果取关成功则将数据也从Redis中移除
